...
def main():
	...
	for res in m_tr:
		...
		fund_headers['path'] = '/api/coin-profile/fund?currency=cny&coin_type='+code
		fund_querystring['coin_type'] = code
		fund_response = requests.request("GET", fund_url, headers=fund_headers, params=fund_querystring)
		try:
			fund_response = fund_response.json()
			fund_data = fund_response['flow_distribute']
			sql = "replace into coin_fund (code,fund) value('"+code+"','"+json.dumps(fund_data)+"')"
			connect['cur'].execute(sql)
			connect['con'].commit()
		except:continue
