...
// 		error_log("\n");
	...
	/**
	 * wrap split pages params.
	 * @return array | total:总记录数 pageSize: 单页记录数 totalPage: 总页数
	 */
	public static function splitPage($total, $pageSize = 20) {
  		$totalPage = ceil($total/$pageSize); //总页数 
  		$resultarr['total'] = intval($total); 
		$resultarr['pageSize'] = $pageSize; 
	 ...
	 * 对象转数组
	 */
	public static function obj2ary($data) {
	    if(is_array($data) || is_object($data)) {
	        $result = array();
	        foreach($data as $key => $value) {
	            $result[$key] = self::obj2ary($value);
	        }
	        return $result;
	    }
	    return $data;
	...
	/**
	 * 解析apk文件
	 * @param  [type] $apk_file [description]
	 * @return [type]           [description]
	 */
	public static function parseApk($apk_file) {
		$appObj  = new Apkparser(); 
        $res   = $appObj->open($apk_file);
        return array(
        	'name' => $appObj->getAppName(),
        	'package' => $appObj->getPackage(),
        	'ver_name' => $appObj->getVersionName(),
        	'ver_code' => $appObj->getVersionCode()
        );
	}
}
